...
`islands.md` §3(e): *every rule here is `conditions → consequent`; only the consequent and the executor
...
**⭐⭐ A TAGGED SHAPE, not its own grammar — and that was the open question** (`HANDOFF.md` §9 item 1:
*"whether a consequent is a fourth tagged shape or its own small grammar"*). Measured by
`docs/microfunctions/probe_consequent.py`, and the tag wins for a reason that only shows up when you push
to execution:
...
`islands.md` §3(b)'s island, created by the second caller, four more times.
...
the same question, and `islands.md` §4 already records that a seam is worth more than a merge.
"""
...
#: adding a tag with nothing that runs it is `islands.md`'s EXECUTION verdict, which is worse than no form.
