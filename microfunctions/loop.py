...
**The principle, which is the user's and is sharper than what the engine was built toward** (HANDOFF §6b):
...
def schedule(g: Graph, loop: str, task: str, why: str | None = None) -> str:
...
def verb_of(g: Graph, task: str) -> str:
    ...
    `act` according to whether the tool was registered as observing, which is the distinction HANDOFF §6b
...
# --- the loop -----------------------------------------------------------------------------------------
def tick(g: Graph, loop: str, **hooks):
...
def run(g: Graph, loop: str, *, max_ticks: int = 10_000, until=None, **hooks) -> dict:
    ...
    ⚠ `max_ticks` is a budget, and it is now the ONLY one above the ISA's own. HANDOFF §6b's argument for
...
__all__ = ["KINDS", "IMAGINE", "LOOK", "ACT", "RUN", "FORGET", "IRREVERSIBLE",
           "open_loop", "schedule", "agenda", "ticks", "finished", "describe", "verb_of",
           "advance", "tick", "run"]
