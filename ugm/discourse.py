...
Reading an authored block builds a goal, a criterion or a method, and this is what records that
somebody said it. Without it the system can hold what it has been told and cannot point at the
telling.

An utterance is in the world; the thread merely attends it. Two things separate cleanly. The
conversation holds utterances by agents: it hangs off `root`, its speakers are nodes, another
agent can quote it, and a rule can doubt it — this is where multi-party lives. The system's own
episode records where attention went: metadata, pointing inward, never pointed at. So an
utterance hangs off the conversation and the thread attends it, which needs no new entry kind,
because `thread.attend` already links an entry to a world node.

A speaker is a node, never a string attribute. Identifying by name alone is harmless with one
actor, wrong the moment there are three, and cannot represent an external system or another agent
at all.

Retraction still needs utterances and applications in one order — was this already acted on? — and
it has that, because an attention entry sits in the same ordered edge as everything else.

Retract the utterance, not the world. "Ignore that" looks like one move and is three, with wildly
different costs:

| what is asked | what happens here |
|---|---|
| stop consulting it | this: the block is marked withdrawn and every enumerator skips it |
| undo what it let us conclude | not done; there is no retraction or truth-maintenance machinery |
| undo what was done in the world | not possible; a rollback boundary must never span a dispatch |

Withdrawal is prospective and history is deliberately left standing. A withdrawn criterion that
already shaped a plan is still cited by `why`, annotated as withdrawn. A record saying "this
happened because of something you later took back" is strictly more useful than one with a hole
where the reason was. Withdrawal is a mark rather than a deletion for the same reason, and
`is_withdrawn` is asked at enumeration time by the functions that enumerate authored data, so
nothing has to remember to filter.

The conventions are a protocol, because the graph may be shared. Another program may write into
it, using its own locks and respecting these conventions. To participate, write exactly this:
...
reads off the thread, so an external write is invisible until `attend_new` brings it in.

Two constraints the locking discipline must respect, and they are not ours to enforce. A rollback
boundary must not span an external write, because `intake.read` is savepoint-scoped and rolls
back on refusal, so a concurrent writer inside that window would be undone by a rollback that has
nothing to do with it. And mint-then-link must be atomic to other readers, because an utterance
linked to the conversation before its `by` edge exists is, for one instant, an utterance nobody
said.
...
def _utter(g: Graph, thread: str, *, by, verb: str | None, about: str | None,
    ...
    Two records, and they are not a duplicate — they are two halves that are easy to conflate.
...
def unattended(g: Graph, thread: str) -> tuple:
    ...
    (the user's is that *another piece of software may write into the graph, using its own
...
def ask(g: Graph, thread: str, question: str, *, note: str | None = None) -> dict:
    ...
    * the question is committed before the handler runs, so a host that never answers still leaves a
    ...
    a workbench, so it is tempting to conclude that the planner structurally
    cannot ask. That does not follow, and it is false: the target here is the question node, minted
