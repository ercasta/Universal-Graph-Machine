...
⭐ This is also the caller `docs/limits.md` **G7** was missing. That entry (*"who said so"*, beliefs held
...
* **A rollback boundary must not span an external write.** `intake.read` is savepoint-scoped and rolls back
  on refusal — a property a consumer's feedback depends on for parse-as-you-type — so a concurrent
