                ...
                if name_attr is not None:
                    ...
                    for cand in g.nodes_named(nm):        # insertion-first, like nodes_named(nm)[0]
                        ...
                        if not is_domain_rel:
                            new = cand
                            break
                    # (The write-side token->entity `intern_denoted` mode was RETIRED 2026-07-23: the demand
                    # fetch now reads a bound endpoint as its CANONICAL CLASS (`chain._canon_class`), so a
                    # deferred recognizer's handle can intern to the token and the node-bound join still sees
                    # the entity's folded content — the identity boundary, not a per-site write patch.
