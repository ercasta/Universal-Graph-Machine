...
def _expand_rule_node(graph: Graph, R: str, declared: set[str] = frozenset()) -> Rule:
    ...
    # A closed-world `is not P` clause is UPGRADED from a NAC into a positive `is_not` match — DECIDED
    # NEGATION (decide.py), the ONLY negation model (Phase 6.1). An ordinary `not` clause (not
    # closed-world) stays a NAC (a structural guard, not epistemic negation). The completion rule for
    # each upgraded clause is generated by `_completion_rules` and run by the forward `decide.solve`.
    cw_neg = [c for c in nac_conds if _is_cw_negation(graph, c)]
...
def expand_rules(rule_graph: Graph) -> list[Rule]:
    ...
    DECIDED NEGATION (Phase 6.1, the only model): a rule with a CLOSED-WORLD `is not P` clause is
    reflected as a decided negation — the clause becomes a positive `is_not` condition
    (`_expand_rule_node`) and this tool ALSO emits a companion COMPLETION rule
    (`decide.completion_rule`, aggressive + defeated by INTERPOSE retract, run by `decide.solve`). A
    factless rule graph carries no CWA marker, so neither is emitted — ordinary (non-closed-world)
    NAF guards are unchanged.
    """
...
def load_corpus(text: str) -> tuple[Graph, list[Rule]]:
    ...
    are the executable rules. Reason with `decide.solve(kb, rules)` (DECIDED NEGATION, Phase 6.1 —
    a closed-world `is not P` clause compiles to the completion + INTERPOSE-retract form via
    `expand_rules`; there is no NAF-negation option). Lines starting with '#' are comments.
