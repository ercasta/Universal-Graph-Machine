...
FACT_FORMS: list[Rule] = [
    ...
    Rule(
        ...
        rhs=[Pat("?x", decide.CLOSES, decide.CWA)],
    ),
]
...
def _recognize(graph: Graph, sentences: list[str], forms: list[Rule]) -> list[str]:
    ...
    workaround for the demand-all-heads `solve_all`, which over-recognized globally). NAC guards are
    handled as the driver's match-time filter, keeping the opcode set purely positive.
...
def load_facts(graph: Graph, text: str) -> list[str]:
    ...
    Recognition runs on the ISA forward driver, per-sentence isolated (`_recognize` -> `solve_all`
    + `absorb`), NOT the forward `rewriter` — the "one engine" move (decision-attrgraph-rehost).
...
def _is_cw_negation(graph: Graph, cond: str) -> bool:
    """Is `cond` a CLOSED-WORLD copula negation — an `is not P` body clause whose property `P`
    is declared closed-world? Such a clause is NOT ordinary NAF: it routes through
    `<decide>`/completion (decide.py), so the reflection UPGRADES it from a NAC into a positive
    `is_not` condition + a `<decide>` demand rule, rather than a stratified NAC (handoff step 1).

    The marker must be reachable from the concept node the clause's `k_obj` points at. Under additive
    coreference (`load_corpus` now `wire_same_as`, not the old merge), the fact `P is closed world`
    marks a DIFFERENT `P` mention linked `same_as` to `k_obj`, so `load_corpus` propagates `closes`
    across the links (via `_coref_propagation`, which includes `closes`) BEFORE this reflection reads
    it — the marker lands on `k_obj` just as the merge used to put them on one node. A factless rule
    graph (`load_rules` / `load_machine_rules`) carries no marker, so this is always False there
    and `is not` stays a NAC — the reflection change is backward-compatible by construction."""
    kp, ko = _obj(graph, cond, "k_pred"), _obj(graph, cond, "k_obj")
    return (kp is not None and ko is not None
            and graph.name(kp) == decide.COPULA
            and decide.node_is_closed_world(graph, ko))
...
def _expand_rule_node(graph: Graph, R: str, declared: set[str] = frozenset(),
                      *, decided_negation: bool = True) -> Rule:
    ...
    # A closed-world `is not P` clause is UPGRADED from a NAC into a positive `is_not` match
    # (decided negation, decide.py); ordinary `not` clauses stay NACs (stratified NAF).
    # `decided_negation=False` (the GOAL/ISA path, decision-attrgraph-rehost) SKIPS the upgrade:
    # the closed-world `is not P` stays a plain NAC, which `GoalSolver._lower_nac` turns into
    # demand-driven COMPLETION (sound, no aggressive over-assertion, no retraction) — so the whole
    # `decide` completion+RETRACT machinery is unneeded on the backward engine (decision-cwa-default).
    cw_neg = [c for c in nac_conds if _is_cw_negation(graph, c)] if decided_negation else []
    plain_nac = [c for c in nac_conds if c not in cw_neg]
    lhs_pats = [_cond_pat(graph, k, declared) for k in lhs_conds]
    for c in cw_neg:
        lhs_pats.append(Pat(rule_var_name(graph.name(_obj(graph, c, "k_subj")), declared),
                            decide.NEG_COPULA,
                            rule_var_name(graph.name(_obj(graph, c, "k_obj")), declared)))
    lhs = sorted(lhs_pats, key=_pat_key)
    nac = sorted((_cond_pat(graph, k, declared) for k in plain_nac), key=_pat_key)
...
def expand_rules(rule_graph: Graph, *, decided_negation: bool = True) -> list[Rule]:
    ...
    `decided_negation=True` (default, the forward `decide` engine): a rule with a CLOSED-WORLD
    `is not P` clause is reflected as a decided negation — the clause becomes a positive `is_not`
    condition (`_expand_rule_node`) and this tool ALSO emits a companion COMPLETION rule
    (`decide.completion_rule`, aggressive + defeated by RETRACT). A factless rule graph carries no
    CWA marker, so neither is emitted — ordinary NAF is unchanged.

    `decided_negation=False` (the GOAL/ISA backward engine, decision-attrgraph-rehost): the
    closed-world `is not P` stays a plain NAC and NO completion rules are emitted — `GoalSolver`'s
    demand-driven completion handles the negation soundly with no over-assertion and no retraction,
    which is how the `decide` machinery is RETIRED on the backward path.
    """
    ...
    rules = [_expand_rule_node(rule_graph, R, declared, decided_negation=decided_negation)
             for R in rule_nodes]
    completions = ([c for R in rule_nodes for c in _completion_rules(rule_graph, R, declared)]
                   if decided_negation else [])
    return sorted(rules + completions, key=lambda r: r.key)


def _completion_rules(graph: Graph, R: str, declared: set[str] = frozenset()) -> list[Rule]:
    """The COMPLETION rules for rule node `R`'s closed-world `is not P` clauses.

    Each upgraded clause gets a rule that materializes `?c is_not P` for the tuples that matter —
    scoped by the consumer's POSITIVE residual (`rl_lhs`, which binds the decided subject) and
    guarded by a NAC on `?c is P` (so it fires only where the closed-world positive is unproven, in
    a stratum after P's producers). A clause with no positive trigger to bind its subject is skipped
    (the decision would be un-scoped) — a documented limit: give a decide-consumer at least one
    positive condition. Emits nothing on a factless rule graph (no CWA marker -> no `cw_neg`)."""
    cw_neg = [c for c in _objs(graph, R, "rl_nac") if _is_cw_negation(graph, c)]
    if not cw_neg:
        return []
    trigger = sorted((_cond_pat(graph, k, declared) for k in _objs(graph, R, "rl_lhs")), key=_pat_key)
    bound = {b for p in trigger for t in p.tokens() if (b := _binder(t)) is not None}
    out: list[Rule] = []
    for c in cw_neg:
        subj = graph.name(_obj(graph, c, "k_subj"))
        prop = graph.name(_obj(graph, c, "k_obj"))
        if subj not in bound:
            continue                                     # subject not bound by any trigger -> skip
        sig = "|".join(f"{p.s}/{p.p}/{p.o}" for p in trigger) + f"#{subj}#{prop}"
        key = f"decide.complete.{prop}.{zlib.crc32(sig.encode()) & 0xFFFFFF:06x}"
        out.append(decide.completion_rule(subj, prop, trigger, key))
    return out
...
def load_corpus(text: str, *, decided_negation: bool = True) -> tuple[Graph, list[Rule]]:
    ...
    are the executable rules. Run them with `run_rules(kb, rules)`. Lines starting
    with '#' are comments; blank lines are skipped.

    `decided_negation` (default True, the forward `decide` engine) compiles a closed-world
    `is not P` clause to the aggressive-completion + retract form (`expand_rules`); pass FALSE for
    the backward ISA engine (`solve_all`/`ask_goal`), which keeps it a plain NAC and completes it
    on demand with no over-assertion and no retraction (`decision-attrgraph-rehost`).
    ...
    rules = expand_rules(kb, decided_negation=decided_negation) + expand_loose_from_graph(kb) + prop
