...
def _ingest_gen(kb, rules, utterance, *, policy=None, attention="global", can_ask=False, trace=False,
    ...
    # The COMPARATIVE and POSSIBILISTIC mini-surfaces used to be reachable ONLY through their standalone
    # batch loaders (`comparative.load_comparative` / `uncertainty.load_uncertain`) — never through the
    # modern intake. These routes fold them into `ingest`, so a live session (or `load_kb`/`load_corpus`)
    # can mix `x is more D than y` and `x is likely P` with plain facts. Placed in the FALLBACK, like
    # focus/stance: the canonical grammar has no vocabulary for either and REFUSES them (MEASURED —
    # `x is more beautiful than y` / `cy is likely a thief` route `unrecognized`), so a grammar KB reaches
    # them by fall-through, a non-grammar KB directly. Each is recognized by its OWN pure parser (§D.2),
    # keyword-gated (more/less/than; likely/either/means) so neither claims a plain fact — pinned by
    # `test_at_most_one_router_recognizer_claims_a_surface`. Above the crisp fact route so `x is likely a
    # thief` authors a banded FORK, not a crisp `is_a` fact.
    #
    # (The hedge route was briefly reverted 2026-07-22 because a fork was destroyed by a later fact's
    # incidental GC — root-caused to edgeless scope nodes and FIXED, see `scope-nodes-survive-incidental-
    # gc`. A fork now survives interleaved ingest, so the route is safe.)
    ...
    # HEDGE / POSSIBILISTIC — `x is likely P` / `x is either A or B` / `P means N` authors a banded FORK
    # (an epistemic SCOPE), or extends the hedge LEXICON (`P means N`, an ink fact so the scale lives in
    # the KB). `uncertainty.load_line` reads the lexicon from the KB in scope, so a hedge declared in an
    # EARLIER ingest call already parses (declare-before-use — the intake contract). It authors a SCOPE,
    # never a rule; the scope survives later fact ingestion since the GC fix.
