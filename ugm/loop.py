...
There is a single outer control loop. Planning is not a separate execution loop: its control
state is represented as data, and the outer loop always interleaves and ticks. No seams, and no
fixpoint procedures that cannot be interrupted.

Almost nothing here is mechanism, because every control loop in the engine is already a node plus
a `step`:

| task | one primitive step is | state |
|---|---|---|
| `activation` | one instruction | `activation.py` |
...
| `forgetting` | one swept record | `forget.py` |

So the outer loop is an ordered agenda and a dispatch on kind. Adding a kind of work means
writing its `step`, not touching this file.

Round-robin, and the rotation is the data. `tick` takes the task at the head of the agenda,
advances it by one step and puts it back at the tail, so interleaving is not a policy this module
implements but the ordinary consequence of the agenda being an ordered edge. Two goals pursued at
once really do interleave, one primitive step each, and which one is next is a question anybody
can ask of the graph.

What must not become uniform is irreversibility. Advancing a search costs time; advancing a
replay can send an email, and that asymmetry is the most important safety property in the design.
So a tick reports the verb it performed — imagine, look, act, run — and a caller can stop before
the first irreversible one. Dispatch is not a seam to remove: once a handler is called the world
is executing, not us. The loop can decline to take the step; it cannot make the step reversible.

It does not flatten the goal. A game loop simulates; this pursues. The unmet constraints of a
goal are the gradient and the entire reason guidance works. The loop's shape governs what may
...
#: The verbs, as they arise. `forget` is the slower clock, and it is an ordinary task on this
...
def schedule(g: Graph, loop: str, task: str, why: str | None = None, *, not_before=None) -> str:
    ...
    *"Stop cooking the pasta after ten minutes"* — a procedure installs it on itself, so the payload is an
    ...
    # Two different facts were riding on one edge, and it made a question unanswerable exactly
    # when it mattered. `loop -task-> t` is turn order: `tick` unlinks the head before advancing it and
...
def describe(g: Graph, task: str) -> str:
    """What this task is in the middle of. Every kind can answer, which is the half's test that is
...
def verb_of(g: Graph, task: str) -> str:
    ...
    `act` according to whether the tool was registered as observing, which is the distinction an earlier note
...
def run(g: Graph, loop: str, *, max_ticks: int = 10_000, until=None, at=None, **hooks) -> dict:
    ...
    `max_ticks` is a budget, and it is now the only one above the ISA's own. An earlier note's argument for
