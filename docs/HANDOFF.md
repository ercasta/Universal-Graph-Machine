...
| ⭐⭐⭐ **the agent's representation of ITSELF, and what triggers what** | [self-and-processes.md](self-and-processes.md) — *a design, nothing built*. Without a self node **"I" and "you" are meaningless** and the system cannot be the subject of any fact; a process is a thing and **doing it is a fact**; the phase machine dissolves into derived readings; a worked *"what are you doing?"* trace end to end |
| ⭐⭐⭐ **the live arc — every relation becomes a node** | [facts-as-nodes.md](facts-as-nodes.md) — the universal shape, why **entities have no outgoing edges**, ordered-vs-unordered as declared data, signed frame membership, and the two things that change shape rather than move |
| ⭐ **how a representation decision is scored — the standing criteria** | [harmony.md](harmony.md) — **not leaking / not lossy / readable / composable**, the worked `a on b` table, the one criterion that is mechanically audited (`python -m ugm.leak`), and ⭐ **expressiveness is PRIOR to the four, not a fifth row** |
| ⭐⭐⭐ **the through-line, and eight findings that re-order the plan** | [addressability.md](addressability.md) — *every expressiveness problem here is an addressability problem in disguise*. **Bridges reconcile NAMING, not COVERAGE**; the shared vocabulary is the matrix's **columns**; **procedures are compiled searches**, so the forward direction is still open; the axis map collapses to a stack order plus small tables; **asking is cheaper than deferring**; `why` needs its contrast class as an argument. Eight probes, ordered, two predictions recorded so they can fail |
