...
**Slice 1 — the `holder` kind (attribution).**
...
**Slice 2 — `temporal` (ontological ordered) scopes + rule-binding. The deep piece; do last.**
...
- **How `kind` is represented** — a `kind` valued attr on the scope node vs distinct marker names.
  *Recommendation: a `kind` attr*, extending the `<likeliness>`-on-`<hypothesis>` pattern; dispatch
  the read on it. (Blocks Slice 1.)
