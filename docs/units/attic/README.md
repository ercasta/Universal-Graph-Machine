...
| `handoff_overlay_band_composition.md` | Does possibilistic band composition (`machine.OVERLAY_BAND`) need privileged ISA support, or is it ordinary declared rules?, 2026-07-30 | **RESOLVED the same day, then SELF-CORRECTED the same day** after a direct question ("isn't relativization just a convention — shouldn't the machinery just point at a local subgraph region?") caught an unverified claim. **Verdict stands: CONFIRMED SUGAR (retirement candidate), but the located gap moved.** First pass wrongly claimed `Pat.rel` needed a new `KIND_HYPOTHESIS` dispatch arm; verified by running it that `chain._relativized_st_matching` (the `@?h` reader) already covers fork scopes with zero changes, since forks use the identical `<under>`-edge convention as everything else. The real, verified gap: `chain._grades_pass` (`chain.py:1078`) computes the correct min-composed degree via `GRADE` but discards it (`bool(...)`, pass/fail only), never folding it into `band` — confirmed by a probe rule that fires but reports `"positive"` instead of `"likely"`. Fix is local: thread the achieved score through `_grades_pass` and its one caller. Corrected resolution at the top of the file itself (§6) |
