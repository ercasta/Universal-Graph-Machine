# Forms — the closed class, and what makes it compose

**Status: design, 2026-07-27.** Supersedes `docs/design/form_inventory.md`, which predates the substrate
inversion (that document is 07-20/22; the inversion is 07-26) and whose every *mechanism* column refers to the
retired `ugm` engine. What survives from it is carried here explicitly (§10); nothing else should be read from it.

Written from a working session with you. Your framing drove it, and the three questions that moved it are marked
where they land: *"what are the categories?"* (§2), *"is there a catalog in the literature?"* (§6), and *"how does
'the closed class is small' coexist with 350 years of failure?"* (§3.2) — the last of which produced the result
the document is now organised around.
...
### 3.4 The partial mapping that does exist — skeleton and remainder

The instinct that open-class words reduce is not wrong, only not *exhaustive*. The surviving decompositional
result is that an open-class word has a **closed-class skeleton and an irreducible remainder**:

- *park* contributes an argument structure (agent, patient, location), an aspectual class (accomplishment), and
  a change-of-state pattern — **all closed class**.
- What distinguishes *park* from *stop*, *leave*, *abandon* is **not closed class and not decomposable**.

Fodor's objection (§7.2) is the proof that the remainder is real. This is also what FrameNet / VerbNet / PropBank
...
**The defensible version:** embeddings are an excellent solution to the **open** class and a poor one for the
**closed** class. That predicts the observed failure profile — LLMs are strong at plausible inference over rich
content and weak at exactly the closed-class operations: long chains, negation scope, counting, quantifier
nesting, tracking who committed to what. **That is not a random failure list; it is the closed class.**

⚠ *Why* LLMs do what they do is not settled, and this document claims no theory of it. What is claimed is the
split, and the split is what justifies the architecture: LLM at the boundary for the open class, engine for the
closed class — which is small, which is the only reason this is buildable.
...
Every form in the inventory carries these fields. The middle three are harmony made operational.
...
- **Is the entry format sufficient?** §5's eight fields are designed, not validated. The test is writing ten real
  entries and seeing which field is always empty or always fudged.
